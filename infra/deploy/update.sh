#!/usr/bin/env bash
# =============================================================================
# update.sh — Update ShellKeep on VPS
# =============================================================================
# Location on VPS: /opt/infra/apps/shellkeep/update.sh
#
# This script:
...
#
# Prerequisites:
#   - ShellKeep installed via install-shellkeep.sh
#   - .env file configured
#   - Services running
# =============================================================================
set -euo pipefail
...
COMPOSE_FILE="${SHELLKEEP_DIR}/compose.yml"
PROXY_CONF_SRC="${SHELLKEEP_DIR}/shellkeep.conf"
PROXY_CONF_DST="/opt/infra/proxy/nginx/conf.d/shellkeep.conf"
BRANCH="main"
...
# Helpers
# ---------------------------------------------------------------------------
log() { echo "[shellkeep-update] $(date '+%H:%M:%S') $*"; }

show_help() {
    echo "Usage: $0 [OPTIONS]"
    echo ""
    echo "Update ShellKeep to the latest version."
    echo ""
    echo "Options:"
...
if [ ! -d "${REPO_DIR}/.git" ]; then
    ...
    log "Run install-shellkeep.sh first."
    exit 1
fi
...
if [ ! -f "${COMPOSE_FILE}" ]; then
    ...
    log "Run install-shellkeep.sh to generate it."
    exit 1
fi
...
cat > "${SHELLKEEP_DIR}/.update-state" << EOF
